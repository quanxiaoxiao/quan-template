...
var defaultConfig = {
  ...
  react: {
    ...
    post: function post(name) {
      ...
      var devDependencies = ['autoprefixer', 'babel-core', 'babel-loader', 'babel-plugin-transform-runtime', 'babel-preset-env', 'babel-preset-react', 'babel-preset-react-hmre', 'babel-preset-stage-0', 'cross-env', 'css-loader', 'extract-text-webpack-plugin', 'html-webpack-plugin', 'lodash-webpack-plugin', 'node-sass', 'postcss-loader', 'redux-logger', 'rimraf', 'sass-loader', 'style-loader', 'url-loader', 'webpack', 'webpack-hot-middleware'];
      _shelljs2.default.exec('npm install ' + dependencies.join(' '));
      _shelljs2.default.exec('npm install --save-dev ' + devDependencies.join(' '));
    }
  },
  ...
  pro: {
    ...
    post: function post(name) {
      _shelljs2.default.cd(name);
      _shelljs2.default.sed('-i', /"name":\s+"([^"]+)"/, '"name": "' + name + '"', 'package.json');
      var devDependencies = ['babel-core', 'babel-preset-env', 'babel-preset-stage-0'];
      _shelljs2.default.exec('npm install --save-dev ' + devDependencies.join(' '));
    }
  },
...
function combineConfig(a, b) {
  return Object.keys(a).reduce(function (acc, key) {
    ...
    return _extends({}, acc, _defineProperty({}, key, _extends({}, obj.parent, obj)));
  }, {});
}
...
try {
  var _config = require(_path2.default.resolve(_os2.default.homedir(), 'temp.config.js')); // eslint-disable-line
...
try {
  var _config2 = require(_path2.default.resolve(process.cwd(), 'temp.config.js')); // eslint-disable-line
  selfConfig = combineConfig(_config2, defaultConfig);
} catch (e) {
  // ignore
}

exports.default = _extends({}, defaultConfig, customConfig, selfConfig);